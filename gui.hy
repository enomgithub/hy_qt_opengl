...
(defclass Window [QtWidgets.QWidget]
  (defn --init-- [self &optional [parent None]]
    ...
    (self.x-slider.setValue (* 170 *step*))
    (self.y-slider.setValue (* 160 *step*))
    (self.z-slider.setValue (* 90 *step*))
  ...
  (defn create-slider [self]
    (let [slider (QtWidgets.QSlider QtCore.Qt.Vertical)]
    
      (slider.setRange 0 (* 360 *step*))
      (slider.setSingleStep *step*)
      (slider.setPageStep (* 15 *step*))
      (slider.setTickInterval (* 15 *step*))
...
(defclass GLWidget [QtOpenGL.QGLWidget]
  ...
  #@((QtCore.Slot int)
    (defn setXRotation [self angle]
      (let [-angle (self.normalizeAngle angle)]
  ...
  (defn initializeGL [self]
    ...
    (setv self.object (self.make-object))
  ...
  (defn paintGL [self]
    ...
    (list (map (fn [-rot] (GL.glRotated #* -rot))
               [(, (/ self.x-rot (float *step*)) 1.0 0.0 0.0)
                (, (/ self.y-rot (float *step*)) 0.0 1.0 0.0)
                (, (/ self.z-rot (float *step*)) 0.0 0.0 1.0)]))
  ...
  (defn mouseMoveEvent [self event]
    (let [dx (- (event.x) (self.last-pos.x))
  ...
  (defn make-object [self]
    (let [gen-list (GL.glGenLists 1)
          x1 0.06
          y1 -0.14
          x2 0.14
          y2 -0.06
